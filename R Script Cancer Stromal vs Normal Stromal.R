...
#Dataset: GSE10797 (Breast Cancer Stromal vs Normal Stromal)

#Dataset GSE10797 berisi ekspresi gen dari sel epitel dan stroma
#yang diisolasi menggunakan laser capture microdissection
#dari jaringan payudara normal dan kanker invasif.

#Platform: Microarray (Affymetrix Human Genome U133A 2.0)
...
#antara dua kondisi biologis (misalnya kanker payudara stromal vs normal stromal) 
...
BiocManager::install(c("GEOquery", "limma", "AnnotationDbi"), ask = FALSE, update = FALSE) 
...
stromal_index <- grep("stromal",
                      sample_info$source_name_ch1,
                      ignore.case = TRUE)

gset <- gset[, stromal_index]
...
#1. Menstabilkan varians antar gen
#2. Mendekati distribusi normal (asumsi model linear)
...
if (LogTransform) {
  ...
  ex <- log2(ex)
  exprs(gset) <- ex
  print("Data berhasil di-Log2 Transform.")
}
...
head(sample_info$source_name_ch1)

#make.names(): mengubah teks menjadi format valid untuk R
group <- ifelse(grepl("tumor|cancer",
                      sample_info$source_name_ch1,
                      ignore.case = TRUE),
                "Cancer",
                "Normal")
...
#Volcano plot memvisualisasikan hubungan antara:
#- Log2 fold change (perubahan biologis)
#- Signifikansi statistik (-log10 p-value)
...
ggplot(volcano_data, aes(x = logFC, y = -log10(adj.P.Val), color = status)) +
  ...
  theme_minimal() +
  ggtitle("Volcano Plot DEG GSE10797 Kanker Payudara")


#PART J.2 VISUALISASI HEATMAP 

#Heatmap digunakan untuk memvisualisasikan pola ekspresi
#gen-gen signifikan antar sampel dan melihat clustering
#antara kelompok Cancer dan Normal

#Pilih 50 gen paling signifikan berdasarkan adj.P.Val
top50 <- head(
  topTableResults[order(topTableResults$adj.P.Val),],
  50
)

#Ambil matriks ekspresi untuk gen terpilih
mat_heatmap <- ex[match(top50$PROBEID, rownames(ex)), ]
...
write.csv(topTableResults, "Hasil_Analisis_GSE10797_DEG.csv")
